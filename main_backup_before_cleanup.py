...
def create_billing_session(data: BillingSessionModel, request: Request, user: dict = Depends(require_therapist_or_admin)):
...
def create_outcome_measure_endpoint(data: dict):
    """
    ...
    """
    try:
        # Direct database insert to bypass validation issues
        import sqlite3
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (
            str(data.get("patient_id")),
            data.get("therapist", ""),
            data.get("appointment_id"),
            data.get("date"),
            int(data.get("outcome_measure_type_id")),
            data.get("score"),
            data.get("comments", "")
        ))
        ...
        print(f"Error creating outcome measure: {e}")
...
def add_outcome_measure_subscores_endpoint(measure_id: str, subscores: List[dict] = Body(...)):
    ...
    try:
        # Direct database insert to bypass module issues
        import sqlite3
        conn = sqlite3.connect("data/bookings.db")
        cursor = conn.cursor()
        
        # First verify the measure exists
        check = cursor.execute("SELECT id FROM outcome_measures WHERE id = ?", (measure_id_int,)).fetchone()
        if not check:
            conn.close()
        ...
        for subscore in subscores:
            if subscore.get("score") is not None:  # Only insert if there's a score
                cursor.execute("""
        ...
        return {"status": "success", "message": "Subscores added successfully"}
        
    except HTTPException:
        raise
    except Exception as e:
        print(f"Error adding subscores: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to add subscores: {str(e)}")


# ===== NEW OUTCOME MEASURES API FOR TREATMENT NOTES INTEGRATION =====

@app.get("/api/test-endpoint")
def test_endpoint():
    """Test endpoint to verify server is picking up new routes"""
    return {"status": "working", "message": "New endpoints are loaded"}

@app.get("/api/appointments/{appointment_id}/outcome-measures")
def get_appointment_outcome_measures(appointment_id: str):
    """
    Get all outcome measures for a specific appointment
    """
    try:
        ...
        for measure in measures:
            ...
            """
            subscore_cursor = conn.execute(subscore_query, (measure['id'],))
            measure['subscores'] = [dict(row) for row in subscore_cursor.fetchall()]
        
        conn.close()
        return {"measures": measures}
        
    except Exception as e:
        print(f"Error in get_appointment_outcome_measures: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to get outcome measures: {str(e)}")


@app.get("/api/outcome-measures/{measure_id}")
def get_outcome_measure_endpoint(measure_id: int):
    """
    Get a specific outcome measure with subscores
    """
    try:
        measure = get_outcome_measure_by_id(measure_id)
        if not measure:
            raise HTTPException(status_code=404, detail="Outcome measure not found")
        
        # Add subscores and type information
        from modules.outcome_measures import get_outcome_measure_subscores
        measure['subscores'] = get_outcome_measure_subscores(measure_id)
        
        # Get type information
        conn = sqlite3.connect("data/bookings.db")
        type_info = conn.execute("""
            SELECT name, category, description, max_score, interpretation
            FROM outcome_measure_types 
            WHERE id = ?
        """, (measure['outcome_measure_type_id'],)).fetchone()
        conn.close()
        
        if type_info:
            measure.update({
                'type_name': type_info[0],
                'category': type_info[1], 
                'type_description': type_info[2],
                'max_score': type_info[3],
                'interpretation': type_info[4]
            })
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get outcome measure: {str(e)}")


@app.put("/api/outcome-measures/{measure_id}")
def update_outcome_measure_endpoint(measure_id: int, data: dict):
    """
    Update an existing outcome measure
    """
    try:
        result = update_outcome_measure(measure_id, data)
        return result
    except HTTPException:
        raise
    except Exception as e:
...
@app.delete("/api/outcome-measures/{measure_id}")
def delete_outcome_measure_endpoint(measure_id: int):
    """
    Delete an outcome measure
    """
    try:
        result = delete_outcome_measure(measure_id)
        return result
    except HTTPException:
        raise
    except Exception as e:
